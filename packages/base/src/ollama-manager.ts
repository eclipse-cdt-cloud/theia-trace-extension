...
export class OllamaManager {
    ...
    public async sendToOllama(content: string): Promise<void> {
        ...
        try {
            ...
            if (!models.some((m: { name: string }) => m.name.replace(':latest', '') === this.model.replace(':latest', ''))) {
                throw new Error(`Model ${this.model} is not available locally. Available models: ${models.map((m: { name: string }) => m.name).join(', ')}`);
            }
    ...
    private async showOllamaResponse(response: string): Promise<void> {
        ...
        try {
            contentDiv.innerHTML = await marked.parse(response);
        } catch (error) {
