...
A [full implementation](#annex-a) of this solution is available as part of this ADR.
...
The following code is a full working copy of `xy-output-component.tsx` where D3 was used to implement the charts.

It contains the changes discussed in this ADR, plus the necessary adjustments to allow it to run in Theia Trace Extension, as it was in the date this ADR was published.
...
export class XYOutputComponent extends AbstractTreeOutputComponent<AbstractOutputProps, XYOuputState> {
    ...
    drawD3Chart(): JSX.Element {
        ...
        if (this.state.xyData.labels?.length > 0) {
            ...
            const xScale2 = scaleLinear()
                .domain([Number(this.props.viewRange?.getStart()), this.props.viewRange?.getEnd()].map(Number))
                .range([0, chartWidth]);
    ...
    private getTimeForX(x: number): bigint {
        ...
        const time = this.props.viewRange.getStart() - offset +
            BIMath.round(x / this.chartRef.current.width * Number(duration));
        return time;
    }
    ...
    protected getXForTime(time: bigint): number {
        ...
        const start = this.props.viewRange.getStart();
        const duration = this.props.viewRange.getDuration();
        const x = Number(time - start) / Number(duration) * this.chartRef.current.width;
        return x;
    }
